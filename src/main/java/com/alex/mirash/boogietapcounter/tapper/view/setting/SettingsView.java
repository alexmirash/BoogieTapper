    ...
    private void init() {
        ...
        initSettingSpinner(tapModeSpinner, R.id.settings_tap_mode_label, settings.getTapMode().ordinal(),
                new OnItemSelectedListener() {
                    ...
                    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                        GlobalSettings.get().setTapMode(SettingTapMode.getValue(position));
                        Log.d("LOL", "tap mode selected: " + GlobalSettings.get().getTapMode());
                    }
                });
        initSettingSpinner(tempUnitSpinner, R.id.settings_unit_label, settings.getUnit().ordinal(),
                new OnItemSelectedListener() {
                    @Override
                    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                        GlobalSettings.get().setUnit(SettingUnit.getValue(position));
                        Log.d("LOL", "unit selected: " + GlobalSettings.get().getUnit());
                    }
                });
        initSettingSpinner(refreshTimeSpinner, R.id.settings_refresh_time_label, settings.getAutoRefreshTime().ordinal(),
                new OnItemSelectedListener() {
                    @Override
                    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                        GlobalSettings.get().setAutoRefreshTime(SettingAutoRefreshTime.getValue(position));
                        Log.d("LOL", "refresh time selected: " + GlobalSettings.get().getAutoRefreshTime());
                    }
                });
    }
