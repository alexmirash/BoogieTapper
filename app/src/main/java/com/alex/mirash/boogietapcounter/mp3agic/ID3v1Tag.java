package com.alex.mirash.boogietapcounter.mp3agic;
...
public class ID3v1Tag implements ID3v1 {
    ...
    public static String trimStringRight(String value) {
        return value == null ? null : BufferTools.trimStringRight(value);
    }

    private void unpackTag(byte[] bytes) throws NoSuchTagException {
        sanityCheckTag(bytes);
        title = trimStringRight(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, TITLE_OFFSET, TITLE_LENGTH));
        artist = trimStringRight(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, ARTIST_OFFSET, ARTIST_LENGTH));
        album = trimStringRight(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, ALBUM_OFFSET, ALBUM_LENGTH));
        year = trimStringRight(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, YEAR_OFFSET, YEAR_LENGTH));
        genre = bytes[GENRE_OFFSET] & 0xFF;
        if (genre == 0xFF) {
            genre = -1;
        }
        if (bytes[TRACK_MARKER_OFFSET] != 0) {
            comment = trimStringRight(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, COMMENT_OFFSET, COMMENT_LENGTH_V1_0));
            track = null;
        } else {
            comment = trimStringRight(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, COMMENT_OFFSET, COMMENT_LENGTH_V1_1));
